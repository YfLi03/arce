...
use crate::api::err;
use crate::api::folders::{ArticleFolder, PictureFolder, ArticleFolderList, PictureFolderList};
...
pub fn add_picture_folder(conn: &Connection, f: PictureFolder)-> Result<(), err::Error>{
    let mut stmt = conn.prepare("INSERT INTO picture_folders\
        (PATH)\
        VALUES (?1)")?;
...
pub fn init (conn: &Connection) -> Result<(), err::Error> {
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS picture_folders (\
        ID      INTEGER     PRIMARY KEY AUTOINCREMENT,  \
        PATH    TEXT        NOT NULL\
        ", [])?;
