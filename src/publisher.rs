use std::{fs::{create_dir_all, copy}, process::Command, thread::{self, sleep}, time::Duration, path::PathBuf, cmp::min, io::Write};
use log::{warn, info};
    ...
    let yaml = 
        match re.find(&content){
            None => {
                ...
                return Err(err::Error::new(err::Reason::ArticleRender, String::from("YAML not found")));
            }
            Some(c) => {
...
fn article_category(articles: Vec<Article>) -> Result<(), err::Error> {
    ...
    let mut categories = articles.clone().into_iter()
        .map(|x| x.category)
        .collect::<Vec<String>>();
    categories.dedup();
    categories.sort();
...
fn article(articles: Vec<Article>) -> Result<(), err::Error>{
    ...
    context.insert("need_nav", &false);
    for a in articles {
        context.insert("page", &Page::new(4, a.title.to_string() + " | " + &config.title));
